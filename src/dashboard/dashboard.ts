...
export default function (client: KiwiiClient) {
  ...
  app.get('/', (_req, res) => {
    res
      .status(200)
      .render(path.resolve(`${process.cwd()}/src/dashboard/Main.ejs`), {
        _client: client,
        i18n,
      });
  });
  app.get('/commands', (_req, res) => {
    res
      .status(200)
      .render(path.resolve(`${process.cwd()}/src/dashboard/ejs/commands.ejs`), {
        _client: client,
        upperFirstButAcceptEmojis,
        beautifyCategories,
        translatePermissions,
  ...
  app.get('*', (_req, res) => {
    res
      .status(404)
      .render(path.join(process.cwd(), 'src', 'dashboard', 'ejs', '404.ejs'));
  });
  app.use('*', (req, res, next) => {
    try {
      req.query['lang'] = req.query['lang'] || req.cookies['lang'] || 'en';
      return next();
