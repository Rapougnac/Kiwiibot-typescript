...
export default class HelpCommand extends Command {
  ...
  public async execute(
          ...
          value: joinArray(
            [
              ...this.client.commands
                .filter(
                  (cmd) => cmd.help.category === category && !cmd.config.hidden
                )
    ...
    if (!args[0]) {
      ...
      if (
        !command &&
        Boolean(possibleCommand ? possibleCommand : possibleCommandAlias)
      ) {
        const notFoudDym = message.guild.i18n.__mf('help.not_found_dym', {
          command: possibleCommand ? possibleCommand : possibleCommandAlias,
        });
        return message.channel.send(`${client.emotes.error} - ${notFoudDym}`);
      } else if (!command)
      ...
      if (description === `${command.help.name}.description` || !description) {
        description =
          command.help.description || message.guild.i18n.__mf('help.no_desc');
      }
      const embed = new MessageEmbed()
        .setColor('ORANGE')
        ...
        .addFields(
          ...
          },
          {
            ...
            value:
              command.help.utilisation?.replace('{prefix}', client.prefix) ??
                ...
                : translatePermissions(
                    ...
                    .map((x) => {
                      ...
                      if (x.match(/Use Vad/g))
                        x.replace(/Use Vad/g, 'Use Voice Activity');
                      if (x.match(/Guild/g)) x.replace(/Guild/g, 'Server');
                      return x;
                    })
                    .join('\n'),
            inline: true,
