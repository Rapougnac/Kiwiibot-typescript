...
export default class SendPinsCommand extends Command {
  ...
  public async execute(_client: KiwiiClient, message: Message): Promise<void> {
    const pinned = [...(await message.channel.messages.fetchPinned())].flat();
    pinned.reverse();
      ...
      pinned
        .map(
          (pin, i) =>
            ...
            } - Auteur: ${typeof pin === 'string' ? null : pin.author.tag}`
        )
        .join('\n')
    );
  }
