  ...
  public override async execute(): Promise<void> {
    ...
    commands.forEach((command) => {
      void (async () => {
        if (!command) return;
        if (!command.global) {
          const guild = await this.client.guilds.fetch('911736666551640075');
          if (['USER'].includes(command.command.type))
            command.command.description = '';
          guild.commands.create(command.command.toJSON());
        } else {
          this.client.application?.commands.create(command.command.toJSON());
        }
      })();
    });
  }
    ...
    try {
      files.forEach((file) => {
        void (async () => {
          ...
          SlashCommand = (
            SlashCommand as unknown as { default: SlashCommandConstructor }
          ).default;
