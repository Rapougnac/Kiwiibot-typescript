...
CommandInteraction.prototype.react = async function (emoji: EmojiIdentifierResolvable): Promise<MessageReaction> {
    ...
    const msg = await (this.client as unknown as ClientApi).api.webhooks(this.client.user!.id, this.token).messages('@original').get();
...
interface ClientApi {
    readonly api: {
        webhooks(userId: string, token: string): {
            messages(id: string): {
