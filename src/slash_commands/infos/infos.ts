  ...
  public constructor(public client: KiwiiClient) {
    super(client, {
      ...
      commandOptions: [
        {
          name: 'user',
          description: 'Get informations about you, or the specified user',
          type: 1,
          options: [
            {
              name: 'target',
    ...
    switch (subcommand) {
      case 'user': {
          ...
          for (const p of platform) {
            switch (p) {
              case 'web':
                ...
                break;
              case 'desktop':
                device += `${interaction.guild?.i18n.__mf('userinfo.desktop', {
        ...
        if (interaction.guild?.available) {
          ...
          if (target.avatar?.startsWith('a_') || member?.premiumSince) {
      ...
      case 'server': {
        if (interaction.guild?.available) {
          ...
          return interaction.reply('This command cannot be used in DMs.');
        }
      }
      case 'role': {
        const { role } = _role;
